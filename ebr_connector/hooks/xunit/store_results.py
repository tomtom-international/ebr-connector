...
"""
Library pushing XUnit results to LHI. Intended as a baseline for any CI systems that do not provide XUnit file ingestion that can be used instead.
...
from ebr_connector.hooks.common.store_results import (
    assemble_build,
    parse_args,
    normalize_string,
)
...
def add_options(parser):
    ...
    """
    parser.add_argument(
        "--testfiles", nargs="+", required=True, help="List of XML files to parse"
    )


def main():
    """
    Provides a CLI interface that takes in XUnit files and returns the results to LHI
    """
    args = parse_args(
        "Send results of a Jenkins build to a LogCollector instance over TCP.",
        add_options,
    )

    xunit_results = assemble_build(args, get_all_xunit_files, [args.testfiles])

    xunit_results.save_logcollect(
        args.logcollectaddr,
        args.logcollectport,
        cafile=args.cacert,

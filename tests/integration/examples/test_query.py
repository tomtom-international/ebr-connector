...
def test_query_failed_tests(data_client):
    ...
    assert response.hits.total == 1 # 1 documents
    assert len(response.hits[0]["br_tests_object"]["br_tests_failed_object"]) == 5
...
def test_query_for_successful_job(data_client):
    ...
    assert not "br_tests_failed_object" in response.hits[0]["br_tests_object"]
    assert not "br_tests_skipped_object" in response.hits[0]["br_tests_object"]
    assert len(response.hits[0]["br_tests_object"]["br_tests_passed_object"]) == 15
